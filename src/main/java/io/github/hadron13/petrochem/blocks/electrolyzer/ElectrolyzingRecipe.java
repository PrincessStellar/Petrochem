...
import io.github.hadron13.petrochem.mixin.accessor.BasinBlockEntityAccessor;
...
public class ElectrolyzingRecipe extends ProcessingRecipe<RecipeInput, EnergyRecipeParams>{
    public int requiredEnergy = 0;

    public ElectrolyzingRecipe(EnergyRecipeParams params) {
        super(PetrochemRecipeTypes.ELECTROLYZING, params);
        requiredEnergy = params.energy;
    }
    ...
    public static boolean match(BasinBlockEntity basin, Recipe<?> recipe) {
        ...
        boolean filterTest = filter.test(recipe.getResultItem(basin.getLevel()
                .registryAccess()));
        if (recipe instanceof ElectrolyzingRecipe basinRecipe) {
    ...
    private static boolean apply(BasinBlockEntity basin, Recipe<?> recipe, boolean test) {
        boolean isBasinRecipe = recipe instanceof ElectrolyzingRecipe;
        ...
        if (isBasinRecipe && !((ElectrolyzingRecipe) recipe).getRequiredHeat()
        ...
        List<SizedFluidIngredient> fluidIngredients =
                isBasinRecipe ? ((ElectrolyzingRecipe) recipe).getFluidIngredients() : Collections.emptyList();
    ...
    @FunctionalInterface
    public interface Factory<R extends ElectrolyzingRecipe> extends ProcessingRecipe.Factory<EnergyRecipeParams, R> {
        R create(EnergyRecipeParams params);
    }

    public static class Builder<R extends ElectrolyzingRecipe> extends ProcessingRecipeBuilder<EnergyRecipeParams, R, ElectrolyzingRecipe.Builder<R>> {
        public Builder(ElectrolyzingRecipe.Factory<R> factory, ResourceLocation recipeId) {
            super(factory, recipeId);
        }

        @Override
        protected EnergyRecipeParams createParams() {
            return new EnergyRecipeParams();
        }

        @Override
        public ElectrolyzingRecipe.Builder<R> self() {
            return this;
        }

        public ElectrolyzingRecipe.Builder<R> energy(int val){
            params.energy = val;
            return this;
        }
    }
